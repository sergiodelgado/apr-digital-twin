...
function estimateSamplingMinutes(rows: TelemetryRecord[]): number | null {
  ...
  const diffs: number[] = [];
  for (let i = 1; i < times.length; i += 1) diffs.push((times[i] - times[i - 1]) / 60000);
  return median(diffs);
}
...
export function buildExecutiveSummary(
  ...
  if (pressureRatio !== null) {
    if (pressureRatio >= 0.95) { pressureState = "En objetivo"; pressureTone = "OK"; }
    else if (pressureRatio >= 0.9) { pressureState = "En observación"; pressureTone = "WARNING"; }
    else { pressureState = "En riesgo"; pressureTone = "CRITICAL"; }
  }
  ...
  if (minTank !== null) {
    if (minTank <= TANK_CRITICAL_PCT) { tankRisk = "Alto"; tankTone = "CRITICAL"; }
    else if (minTank <= TANK_LOW_PCT) { tankRisk = "Medio"; tankTone = "WARNING"; }
    else { tankRisk = "Bajo"; tankTone = "OK"; }
  }
  ...
  if (maxTurb !== null) {
    if (maxTurb >= TURBIDITY_CRITICAL_NTU) { waterRisk = "Alto"; waterTone = "CRITICAL"; }
    else if (maxTurb > TURBIDITY_ALERT_NTU) { waterRisk = "Medio"; waterTone = "WARNING"; }
    else { waterRisk = "Bajo"; waterTone = "OK"; }
  }
  ...
  if (completeness !== null) {
    ...
    if (completeness >= 95 && (imputed === null || imputed <= 5)) {
      completenessDetail = "Confiable"; completenessTone = "OK";
    } else if (completeness >= 90) {
      completenessDetail = "En observación"; completenessTone = "WARNING";
    } else {
      completenessDetail = "En riesgo"; completenessTone = "CRITICAL";
    }
